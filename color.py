...
# ---------------- Paths (files in same folder as script) ----------------
...
# Candidate (adjust if needed)
CANDIDAT_ID = "C001"
CANDIDAT_NOM = "Candidat 1"

# ---------------- Color mapping (your rule) ----------------
COLOR_TO_GROUPE = {
    "FFFF0000": "PLAISIR",   # rouge
    "FF92D050": "RESULTAT",  # vert
    "FF00B0F0": "MAITRISE",  # bleu
}
...
# =========================
# 1) Competences file (openpyxl to read colors)
# =========================
...
for sh in wb.sheetnames:
    ...
    # Title is in C1 (row=1 col=3) in your file
    ...
    col_to_cat = {}
    empty_streak = 0
    for col in range(FIRST_COL, 80):  # up to column CB, safe
        val = ws.cell(row=CAT_ROW, column=col).value
        if val is None or str(val).strip() == "":
            empty_streak += 1
            # stop after many empty columns in a row
            if empty_streak >= 8:
                break
            continue
    ...
    # If no categories detected, skip sheet
    if not col_to_cat:
        continue

    # ---- Scan competence zone under categories: rows 4.. (stop on empty streak)
    # We include ONLY colored cells (solid fill + rgb in COLOR_TO_GROUPE).
    empty_row_streak = 0
    for r in range(CAT_ROW + 1, CAT_ROW + 250):  # scan enough rows
        any_value_in_row = False
        ...
        for col, cat_id in col_to_cat.items():
            ...
            any_value_in_row = True

            fill = cell.fill
            # Only keep real colored cells
            ...
            if rgb not in COLOR_TO_GROUPE:
                continue  # not selected / unknown color
...
# =========================
# 2) Finalites file (pandas is fine: it's tabular)
# =========================
...
for i in range(df_fin.shape[0]):
    ...
    # Theme header row: col0=theme name, col1="Attention", col2="Important"
